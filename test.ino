...
static const u4_t DEVADDR = 0x020106AA; // <-- Change this address for every node!
...
uint8_t mydata[] = "No wifi spots found";
uint8_t macs[28];  // Array to store the 4 mac for
uint8_t rssi[4];   // Array to store the 4 rssi in char
char buffer[64];
...
boolean hasLocation() {
  ...
  if (n == 0) {
    ...
    Serial.print(n);
    Serial.println("Yup, found some Wifi spots :D");
    for (int i = 0; i < n; ++i) {
      // Send max 4 macs + rssi
      unsigned char te = (char)WiFi.BSSID(i)[0]; // TODO save it in rssi array
      Serial.println(te);
      // TODO, scan more or make it random to save data
      if(i < 4) {
        Serial.print(WiFi.SSID(i));
        Serial.println();
        Serial.print(WiFi.BSSID(i)[0]);
        Serial.print(":");
        Serial.print(WiFi.BSSID(i)[1]); 
        Serial.print(":");
        Serial.print(WiFi.BSSID(i)[2]);
        Serial.print(":");
        Serial.print(WiFi.BSSID(i)[3]);
        Serial.print(":");
        Serial.print(WiFi.BSSID(i)[4]);
        Serial.print(":");
        Serial.print(WiFi.BSSID(i)[5]);
        rssi[i] = (uint8_t)((-1) * WiFi.RSSI(i)); // convert RSSI it to a positive char
        for(int c = 0; c < 5; c++) {
            macs[(i * 6) + c] = WiFi.BSSID(i)[c];
        }
      }
      delay(10);
    }
...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & (1 << 7)) {
      ...
      datarate++;
      if (datarate>7) { datarate=0; }
      //LMIC_setDrTxpow((dr_t)datarate,17);
      delay(10);
      if(hasLocation()) {
        LMIC_setTxData2(1, macs, sizeof(macs)-1, 0);
      } else {
...
void setup() {
  ...
  //
  LMIC_setupBand (BAND_MILLI, 14, 10);
  LMIC_setupBand (BAND_CENTI, 14, 10);
  // 
  delay(100);
  // 
  Serial.flush();
